...
namespace WinFormsApp4
{
    ...
    {      
        ...
        private void Ô‡‚Í‡ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ÓÚÏÂÌËÚ¸ToolStripMenuItem.Enabled = textBox1.CanUndo ? true : false;

            ‚˚ÂÁ‡Ú¸ToolStripMenuItem.Enabled = textBox1.SelectionLength > 0 ? true : false;

            ÍÓÔËÓ‚‡Ú¸ToolStripMenuItem.Enabled = textBox1.SelectionLength > 0 ? true : false;

            ‚ÒÚ‡‚ËÚ¸ToolStripMenuItem.Enabled = Clipboard.GetDataObject().GetDataPresent(DataFormats.Text) ? true : false;

            Û‰‡ÎËÚ¸ToolStripMenuItem.Enabled = textBox1.TextLength > 0 ? true : false;

            Ì‡ÈÚËToolStripMenuItem.Enabled = textBox1.TextLength > 0 ? true : false;

            Ì‡ÈÚËƒ‡ÎÂÂToolStripMenuItem.Enabled = textBox1.TextLength > 0 && DataClass.form2_search.Visible ? true : false;

            Ì‡ÈÚË–‡ÌÂÂToolStripMenuItem.Enabled = textBox1.TextLength > 0 && DataClass.form2_search.Visible ? true : false;

            Á‡ÏÂÌËÚ¸ToolStripMenuItem.Enabled = textBox1.TextLength > 0 ? true : false;

            ÔÂÂÈÚËToolStripMenuItem.Enabled = textBox1.Lines.Length > 1 ? true : false;

            ‚˚‰ÂÎËÚ¸¬ÒÂToolStripMenuItem.Enabled = textBox1.TextLength > 0 ? true : false;
        }

        private void Ì‡ÈÚËƒ‡ÎÂÂToolStripMenuItem_Click(object sender, EventArgs e)
        {
            DataClass.form2_search.ToDownRadioButton.Checked = true;
            DataClass.form2_search.buttonSearch_Click(sender, e);
        }

        private void Ì‡ÈÚË–‡ÌÂÂToolStripMenuItem_Click(object sender, EventArgs e)
        {
